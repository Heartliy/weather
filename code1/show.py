...
def get_connect():  # 获取连接
    connect = pymysql.Connect(host='localhost', port=3306, user='root', passwd='abc123456', db='weather', charset='utf8')
    return connect
...
if None in TMAX:
    ...
    # 处理列表中不存在None值的情况
    # 或者您可以选择忽略处理此情况，根据您的需求来决定是否需要进行处理
    pass
...
plt.xlabel('年份(1947-2023) / 年', fontproperties='SimHei', fontsize=15, color='green')
...
plt.xlabel('年份(1947-2023) / 年', fontproperties='SimHei', fontsize=15, color='red')
